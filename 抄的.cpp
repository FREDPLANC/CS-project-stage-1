...
class centerNode {
    public:
        ...
        centerNode(T value):key(value), degree(0), marked(false),
...
class centerHeap {
    ...
    public:
        ...
        void removeMin();
        // 将other合并到当前堆中
        void combine(centerHeap<T> *other);
    ...
    private:
        // 将node从双链表移除
        void removeNode(centerNode<T> *node);
        // 将node堆结点加入root结点之前(循环链表中)
        void addNode(centerNode<T> *node, centerNode<T> *root);
        // 将双向链表b链接到双向链表a的后面
        void catList(centerNode<T> *a, centerNode<T> *b);
        // 将节点node插入到斐波那契堆中
        void insert(centerNode<T> *node);
        // 将"堆的最小结点"从根链表中移除，
        centerNode<T>* extractMin();
        // 将node链接到root根结点
        void link(centerNode<T>* node, centerNode<T>* root);
        // 创建consolidate所需空间
        void makeCons();
        // 合并斐波那契堆的根链表中左右相同度数的树
        void consolidate();
        // 修改度数
        void renewDegree(centerNode<T> *parent, int degree);
        // 将node从父节点parent的子链接中剥离出来，并使node成为"堆的根链表"中的一员。
        void cut(centerNode<T> *node, centerNode<T> *parent);
        // 对节点node进行"级联剪切"
        void cascadingCut(centerNode<T> *node) ;
        // 将斐波那契堆中节点node的值减少为key
        void decrease(centerNode<T> *node, T key);
        // 将斐波那契堆中节点node的值增加为key
        void increase(centerNode<T> *node, T key);
        // 更新斐波那契堆的节点node的键值为key
        void update(centerNode<T> *node, T key);
        // 在最小堆root中查找键值为key的节点
        centerNode<T>* search(centerNode<T> *root, T key);
        // 在斐波那契堆中查找键值为key的节点
        centerNode<T>* search(T key);
        // 删除结点node
        void remove(centerNode<T> *node);
        // 销毁斐波那契堆
        void destroyNode(centerNode<T> *node);
        // 打印"斐波那契堆"
        void print(centerNode<T> *node, centerNode<T> *prev, int direction);
};
